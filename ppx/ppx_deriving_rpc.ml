...
(* [is_option typ] returns true of the type 'typ' is an option type. 
...
module Of_rpc = struct
  ...
  and expr_of_typ quoter typ =
    ...
    | [%type: [%t? typ] list] -> [%expr function | Rpc.Enum l -> map_bind [%e expr_of_typ quoter typ] [] l | y -> Result.Error (Printf.sprintf "Expecting Rpc.Enum, but found '%s'" (to_string y)) ]

    | [%type: [%t? typ] array] -> [%expr function | Rpc.Enum l -> map_bind [%e expr_of_typ quoter typ] [] l >>= fun x -> return (Array.of_list x) | y -> Result.Error (Printf.sprintf "Expecting Rpc.Enum, but found '%s'" (to_string y))]

    | {ptyp_desc = Ptyp_tuple typs } ->
      let pattern = List.mapi (fun i _ -> pvar (argn i)) typs in
      [%expr function | Rpc.Enum [%p plist pattern] -> [%e of_typ_fold quoter tuple typs] | y -> Result.Error (Printf.sprintf "Expecting Rpc.Enum, but found '%s'" (to_string y))]
    ...
    | { ptyp_desc = Ptyp_variant (fields, _, _); ptyp_loc } ->
        ...
        tags |> List.map (fun field ->
            ...
            | Rtag (label, attrs, false, [ { ptyp_desc = Ptyp_tuple typs }]) ->
              Exp.case
                ...
                (of_typ_fold quoter (fun x -> Exp.variant label (Some (tuple x))) typs)
            | Rtag (label, attrs, false, [typ]) ->
              Exp.case
                [%pat? Rpc.Enum [Rpc.String [%p pstr (attr_name label attrs)]; y]]
                [%expr [%e expr_of_typ quoter typ] y >>= fun x ->
      ...
      and inherits_case =
        ...
        List.fold_left (fun expr typ ->
            [%expr
              match [%e expr_of_typ quoter typ] r with
    ...
    | { ptyp_desc = Ptyp_variant (fields, _, _); ptyp_loc } ->
      let cases =
        fields |> List.map (fun field ->
            ...
            | Rtag (label, attrs, false, [{ ptyp_desc = Ptyp_tuple typs }]) ->
              let l = list (List.mapi (fun i typ -> app (expr_of_typ quoter typ) [evar (argn i)]) typs) in
              Exp.case
            ...
            | Rtag (label, attrs, false, [typ]) ->
              Exp.case
                ...
                [%expr Enum ( (String ([%e str (attr_name label attrs)])) :: [ [%e expr_of_typ quoter typ] x])]
            | Rinherit ({ ptyp_desc = Ptyp_constr (tname, _) } as typ) ->
              Exp.case
                [%pat? [%p Pat.type_ tname] as x]
                [%expr [%e expr_of_typ quoter typ] x]
            | _ ->
              raise_errorf ~loc:ptyp_loc "%s cannot be derived for %s"
                deriver (Ppx_deriving.string_of_core_type typ))
      in
