...
struct
  ...
  type rpc = {
    ...
    (* Capitalized module path (e.g. ["Sr"; "Foo"] for an external
       defined as Sr.Foo.funct *)
  ...
  let rec contains_rpc module_item =
    ...
    | Rpc _ -> true
    | Namespace (_,mis) ->
      List.fold_left (fun a b -> a || b) false (List.map contains_rpc mis)
  ...
  let make_client rpcs =
    ...
    let create_call rpc =
      ...
      <:expr<
        ...
        ) <:expr< Args.$arg_path$.$lid:MyRpc.call_of rpc.fname$ >> rpc.args$
      >>
    in
    ...
    let create rpc =
      ...
      <:expr<
        ...
        R.bind (R.rpc call) (fun response ->
        if response.Rpc.success
        then R.return (Args.$arg_path$.response_of_rpc response.Rpc.contents)
        else
    ...
    in
    <:str_item<
      module ClientM = functor(R: RPCM) -> struct $list: gen_str_items rpcs$ end
    >>

(* Generates a signature for a server module - again, very similar to
   the original idl module, but in this one a 'context' is defined - a
   way of passing in any extra info to the functions called. This
   extra info is passed into the 'process' function in the module
   generated by the functor below *)
let make_server_sig rpcs withmonad =
  let _loc = Ast.Loc.ghost in

  let gen_server_sig r =
    let _loc = r.loc in
    <:sig_item<
      value $lid:r.fname$ : context ->
        $List.fold_left (fun accu arg ->
         ...
         ) (if withmonad then <:ctyp< t $r.rtype$ >> else r.rtype) (List.rev r.args)$;
     >>
   in
   ...
   let rec gen_sig_items items = List.map
    ...
    if withmonad
    then <:str_item<
           module type Server_implM = sig
             type t 'a;
             value bind : t 'a -> ('a -> t 'b) -> t 'b;
             value return : 'a -> t 'a;
             value fail : exn -> t 'a;
             value handle_failure : (unit -> t 'b) -> (exn -> t 'b) -> t 'b;
             type context;
             $list: gen_sig_items rpcs$;
           end >>
    else <:str_item<
...
(* Make the functor that generates server modules. The generated
   module will contain a single function - 'process' - which takes an
   Rpc.call and unmarshals the arguments and passes them to the
...
let make_server_functor version rpcs =
  let gen_match_case rpc =
    ...
    let pattern_list =
      List.rev (MyRpc.list_foldi (fun accu e i ->
    ...
    let inner =
      ...
      let default =
        <:expr<
          Impl.bind ($apply$) (fun x -> Impl.return ($arg_path$.rpc_of_response x))
        >> in

      if rpc.fname = "get_version"
      then match version with
        | Some v -> <:expr< Impl.return ($arg_path$.rpc_of_response ($str:v$)) >>
        | None   -> default
      else default
    ...
    <:match_case<
        ($str:rpc.name$,$MyRpc.patt_list_of_list rpc.loc pattern_list$) -> $outer$
      | ($str:rpc.name$,_) ->
        Impl.fail (Message_param_count_mismatch
          ($str:rpc.name$,$`int:List.length pattern_list$,List.length call.Rpc.params))
    >>
  in
...
let make_orig_server_functor rpcs =
  let _loc = Ast.Loc.ghost in
  <:str_item<
   module Server = functor (Impl : Server_impl) -> struct
     module ImplM = struct
       ...
       value return a = a;
       value fail = raise;
       value handle_failure f g = try f () with e -> g e;
       include Impl;
     end;
...
let rec filter_types si =
  match si with
  | <:str_item< type $lid:lid$ = $body$ >>              -> add_rpcs si lid body
  | <:str_item@_loc< module $foo$ = struct $sis$ end >> ->
    <:str_item<
      module $foo$ = struct
        $list:List.map filter_types (Ast.list_of_str_item sis [])$
      end>>
  | <:str_item@_loc< external $fname$ : $ctyp$ = $override$ >> -> <:str_item< >>
  | <:str_item@_loc< exception $_$ >>                          -> <:str_item< >>
  | _ -> si
...
and add_rpcs si lid body =
    ...
    <:str_item<
      $si$;
      value rec $MyRpc.Rpc_of.gen_one (lid,[],body)$
      and $MyRpc.Of_rpc.gen_one (lid,[],body)$;
    >>;;
...
AstFilters.register_str_item_filter begin fun si ->
  ...
  let (orig_rev_rpcs,_) =
    List.fold_left find_rpcs ([],[]) (Ast.list_of_str_item si []) in
  ...
  let exnty =
    <:ctyp<
      [ $Ast.tyOr_of_list (List.map (fun (uid,ty) ->
          match ty with
          | Some ty -> <:ctyp< $uid:uid$ of $ty$ >>
          | None -> <:ctyp< $uid:uid$ >>) rev_exns)$ ]
     >>
  in

  let exns =
    <:str_item<
      module Exception = struct type exnty = $exnty$ ;
  ...
  let exnty_of_exn =
    let gen_mc = function
      | (uid,Some _) -> <:match_case< $uid:uid$ x -> Exception.$uid:uid$ x >>
      | (uid,None) -> <:match_case< $uid:uid$ -> Exception.$uid:uid$ >> in
    let generic =
      <:match_case< e -> Exception.Internal_error (Printexc.to_string e) >> in
  ...
  let exn_of_exnty =
    let gen_mc = function
      | (uid,Some _) -> <:match_case< Exception.$uid:uid$ x -> $uid:uid$ x >>
      | (uid,None) -> <:match_case< Exception.$uid:uid$ -> $uid:uid$ >> in
    let ors = List.map gen_mc rev_exns in
    <:str_item< value exn_of_exnty x = match x with [ $Ast.mcOr_of_list ors$ ] >>
  in

  let rpcf_type = <:str_item<
    module type RPCM = sig
      type t 'a;
      value rpc : Rpc.call -> t Rpc.response;
      value bind : t 'a -> ('a -> t 'b) -> t 'b;
      value return : 'a -> t 'a;
      value fail : exn -> t 'b;
    end >>
  ...
  let rpc_type = <:str_item<
    module type RPC = sig
      value rpc : Rpc.call -> Rpc.response;
    end >>
  in

  let client = <:str_item<
    module Client = functor(R : RPC) -> struct
      module RPCM = struct
        type t 'a = 'a;
        value rpc = R.rpc;
        value bind a f = f a;
  ...
  <:str_item< $list: (List.map filter_types sis)
              ...
              @ [ exns;
                  ...
                  make_client rpcs;
                  client;
                  make_server_sig orig_rpcs false;
                  make_server_sig orig_rpcs true;
                  make_server_functor version flat_rpcs;
                  make_orig_server_functor orig_rpcs ] $ >>
end
