...
struct
  ...
  let expr_list_of_list _loc exprs =
    ...
    | []   -> <:expr< [] >>
    | h::t -> List.fold_left
                (fun accu x -> <:expr< [ $x$ :: $accu$ ] >>) <:expr< [ $h$ ] >> t

  let patt_list_of_list _loc patts =
    match List.rev patts with
    | []   -> <:patt< [] >>
    | h::t -> List.fold_left
                (fun accu x -> <:patt< [ $x$ :: $accu$ ] >>) <:patt< [ $h$ ] >> t
  ...
  let apply _loc fn fn_i create id modules t a =
    ...
    let app expr =
      list_foldi (fun accu _ i -> <:expr< $accu$ $lid:fn_i i$ >>) expr args in
  ...
  let is_string _loc key =
    if key = "string" then
      ...
      <:expr<
        try let ( _ : $lid:key$ ) = $lid:of_rpc key$ (Rpc.String "") in True
        with [ _ -> False ] >>
  ...
  module Rpc_of = struct
    ...
    let rec product get_field t =
      ...
      let bindings = List.map2 (fun pid (f, _) ->
          <:binding< $pid$ = $get_field f$ >>
        ) pids fields in
    ...
    and create id ctyp =
      ...
      | <:ctyp< list ($lid:key$ * $t$) >> when not (is_base key) ->
        ...
        <:expr<
          ...
          let dict = List.map (fun ($pid1$, $pid2$) ->
            ($lid:rpc_of key$ $nid1$, $create nid2 t$)
          ) $id$ in
          if is_a_real_dict then
            Rpc.Dict (List.map (fun
                [ (Rpc.String k, v) -> (k, v)
              | _ -> assert False ]
            ) dict)
          else
        ...
        let pattern (n, t) ctyps =
          ...
          let body =
            if ids = [] then
              ...
              <:expr< Rpc.Enum [ Rpc.String $str:n$
                               :: $expr_list_of_list _loc (List.map2 create ids ctyps)$
                               ] >> in
      ...
      | <:ctyp< option $t$ >> ->
        let new_id, new_pid = new_id _loc in
        <:expr< match $id$ with
                [ Some $new_pid$ -> Rpc.Enum [ $create new_id t$ ]
                | None -> Rpc.Enum []
                ] >>
      ...
      | <:ctyp< array $t$ >> ->
        let new_id, new_pid = new_id _loc in
        <:expr< Rpc.Enum
                (Array.to_list (Array.map (fun $new_pid$ -> $create new_id t$) $id$)) >>

      | <:ctyp< { $t$ } >>              ->
        product (fun field -> <:expr< $id$ . $lid:revmap field$ >>) t

      | <:ctyp< < $t$ > >>              ->
        product (fun field -> <:expr< $id$ # $lid:field$ >>) t
      ...
      | <:ctyp< $lid:t$ $a$ >>          ->
        apply _loc rpc_of rpc_of_i create id None t a
      | <:ctyp< $id:m$ . $lid:t$ $a$ >> ->
        apply _loc rpc_of rpc_of_i create id (Some m) t a
      ...
      <:binding<
        $lid:rpc_of name$ =
          $List.fold_left (fun accu arg ->
              <:expr< fun $lid:rpc_of_polyvar (name_of_polyvar _loc arg)$ -> $accu$ >>
...
module Of_rpc = struct
  ...
  let str_of_id id =
    match id with <:expr@loc< $lid:s$ >> -> <:expr@loc< $str:s$ >> | _ -> assert false
    ...
    let bindings =
      List.map2 (fun pid (n, ctyp) ->
          if is_option ctyp then begin
            ...
            <:binding< $pid$ =
                       try List.assoc $str:n$ $nid$
                       with [ $runtime_exn_error name nid ("Looking for key "^n)$ ] >>
        ) pids fields in
    <:expr< match $id$ with
            [ Rpc.Dict $npid$ -> let $biAnd_of_list bindings$ in $build_all exprs$
            | $runtime_error name id "Dict"$ ] >>

  let rec create name id ctyp =
    let _loc = loc_of_ctyp ctyp in
    match ctyp with
    | <:ctyp< unit >>   -> <:expr< match $id$ with
                                   [ Rpc.Null -> ()
    ...
    | <:ctyp< string >> ->
      <:expr< match $id$ with
              [ Rpc.String x -> x
              | $runtime_error name id "String(string)"$ ] >>

    | <:ctyp< bool >>   ->
      <:expr< match $id$ with [ Rpc.Bool x -> x | $runtime_error name id "Bool"$ ] >>
    ...
    | <:ctyp< list ($lid:key$ * $t$) >> when not (is_base key) ->
      ...
      <:expr<
        ...
        if is_a_real_dict then begin
          match $id$ with
          [ Rpc.Dict d ->
            List.map (fun (key, $pid$) ->
              ($lid:of_rpc key$ (Rpc.String key), $create name nid t$)
            ) d
          | $runtime_error name id "Dict"$ ]
        end else begin
          match $id$ with
          [ Rpc.Enum e -> List.map (fun $pid$ ->
                             $create name nid <:ctyp< ($lid:key$ * $t$) >>$
                          ) e
    ...
    | <:ctyp< Hashtbl.t string $t$ >> ->
      ...
      <:expr< match $id$ with
              [ Rpc.Dict d ->
                ...
                do { List.iter (fun (key,$pid$) ->
                       Hashtbl.add h key $create name nid t$
                     ) d;
                     h }
              | $runtime_error name id "Dict"$ ] >>
      ...
      let pattern (n, t) ctyps =
        ...
        let patt =
          if ids = [] then
            ...
            <:patt< Rpc.Enum [ Rpc.String $str:lowern$
                             :: $patt_list_of_list _loc pids$ ] >> in
    ...
    | <:ctyp< option $t$ >> ->
      ...
      <:expr< match $id$ with
              [ Rpc.Enum [] -> None
              | Rpc.Enum [ $npid$ ] -> Some $create name nid t$
              | $runtime_error name id "Enum[]/Enum[_]"$ ] >>
    ...
    | <:ctyp< $tup:tp$ >> ->
      ...
      <:expr<
        match $id$ with
        [ Rpc.Enum $patt_list_of_list _loc pids$ -> $expr_tuple_of_list _loc exprs$
        | $runtime_error name id "List"$ ]
      >>
    ...
    | <:ctyp< list $t$ >> ->
      ...
      <:expr< match $id$ with
              [ Rpc.Enum $npid$ -> List.map (fun $npid2$ -> $create name nid2 t$) $nid$
              | $runtime_error name id "List"$ ]
      >>
    ...
    | <:ctyp< array $t$ >> ->
      ...
      <:expr<
        match $id$ with
        [ Rpc.Enum $npid$ ->
          Array.of_list (List.map (fun $npid2$ -> $create name nid2 t$) $nid$)
        | $runtime_error name id "List"$ ]
      >>

    | <:ctyp< { $t$ } >> ->
      product name
        (fun n i ctyp -> <:rec_binding< $lid:revmap n$ = $create name i ctyp$ >>)
        (fun es -> <:expr< { $rbSem_of_list es$ } >>) id t

    | <:ctyp< < $t$ > >> ->
      product name
        (fun n i ctyp -> <:class_str_item< method $lid:n$ = $create name i ctyp$ >>)
        (fun es -> <:expr< object $crSem_of_list es$ end >>) id t
    ...
    | <:ctyp< $lid:t$ $a$ >>          ->
      apply _loc of_rpc of_rpc_i (create name) id None t a

    | <:ctyp< $id:m$ . $lid:t$ $a$ >> ->
      apply _loc of_rpc of_rpc_i (create name) id (Some m) t a
...
let gen_one (name, args, ctyp) =
  ...
  <:binding<
    $lid:of_rpc name$ =
      $List.fold_left
        (fun accu arg ->
           <:expr< fun $lid:of_rpc_polyvar (name_of_polyvar _loc arg)$ -> $accu$ >>)
...
let decompose_value accu = function
  | <:sig_item@_loc< value $lid:n$ : $ctyp$ >> ->
    (_loc, [], n, n, args_of_ctyp (decompose_arrows ctyp), return_type ctyp) :: accu
  | _ -> accu
    ...
    let create _loc namespace name wire_name args =
      ...
      let anonymous_exprs = list_foldi
          (fun accu arg i ->
             if arg.kind = `Anonymous then
               <:expr< $lid:rpc_of (argi (n - i))$ $lid:argi (n - i)$ >>
               :: accu else accu)
          [] (List.rev args) in
      ...
      if contains_names args then
        <:expr<
          ...
          Rpc.call $str:wire_name$
            [ (rpc_of_request arg) :: $expr_list_of_list _loc anonymous_exprs$ ]
        >>
      else
        <:expr<
